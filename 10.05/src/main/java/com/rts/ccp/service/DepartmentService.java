...
//    }
	...
	public List<DepartmentDTO> getParticular(List<Long> regionId) {
		List<DepartmentDTO> depList = new ArrayList<>();
		for(Long i:regionId) {
			Iterator<Department> iterator = departmentRepo.findByRegionId(i).iterator();
			while (iterator.hasNext()) {
				Department department = iterator.next();
				DepartmentDTO departmentdto = new DepartmentDTO();
				departmentdto.setDepartmentId(department.getDepartmentId());
				departmentdto.setDepartmentName(department.getRegion().getRegionName()+"-"+department.getDepartmentName());
				departmentdto.setRegion(department.getRegion().getRegionId());
				departmentdto.setRegionName(department.getRegion().getRegionName());
				depList.add(departmentdto);
			}
