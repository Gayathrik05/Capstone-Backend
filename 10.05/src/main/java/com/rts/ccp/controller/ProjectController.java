...
public class ProjectController {
    ...
    public List<ProjectDTO> getProjects(@PathVariable("departmentId")String departmentId) {
        
    	String[] departmentId1 = departmentId.split(",");
    	List<Long> departmentIds = new ArrayList<>();
    	for (String numberString : departmentId1) {
    	    Long number = Long.parseLong(numberString);
    	    departmentIds.add(number);
    	}
     	return projectService.getParticular(departmentIds);
    }
}
